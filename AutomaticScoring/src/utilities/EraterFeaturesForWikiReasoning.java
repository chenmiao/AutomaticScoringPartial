...
public class EraterFeaturesForWikiReasoning {
	...
	static ArrayList<WikiXmlDocument> computeWikiSuperVecs (String superFolderWiki, HashMap wiki_idf) {
...
//				System.out.println("F measure for class index "+i+" is "+temp);
	...
	public static void main (String[] args) {
		...
		String inPrefix="/Users/miaochen/Documents/diss-experiment/proc-corpus/wikianno/xml"; //param 1
		String outPrefix="/Users/miaochen/Documents/diss-experiment/results/wiki-reason-dft"; //param 2
		...
		Toolbox toolbox=new Toolbox(true,true,false,true,false,false,false,true); //param 4
		toolbox.enableTools();
		...
		for (String prompt : Params.prompts) {
			...
			for (String fold : Params.folds) {
				...
				String trainFolderWiki=inPrefix+File.separator+prompt+File.separator+"train"+File.separator+"train"+fold;
				String testFolderWiki=inPrefix+File.separator+prompt+File.separator+"test"+File.separator+"test"+fold;
				String superFolderWiki=inPrefix+File.separator+prompt+File.separator+"super"+File.separator+"super"+fold;
...
//				get wiki vector from the super files
				ArrayList<WikiXmlDocument> superdocs=computeWikiSuperVecs(superFolderWiki, wiki_idf);
				String[] testfiles=new File(testFolderWiki).list(); //here may have problem of .ds_store document
				for (String testfile : testfiles) {
					if (testfile.startsWith(".")) { //means it's a system file
						continue;
					}
...
//					get wiki vector for the test file
					...
					int score=getScoreGivenFilename(file_score, testfile.replaceAll("\\.wikification\\.tagged\\.full\\.xml", ""));
